...
def split_dataset(initial_config):
    ...
    path_output = initial_config[ConfigKW.PATH_OUTPUT]
...
def make_config_list(param_list, initial_config, all_combin, multi_params):
    ...
    if all_combin:
        ...
        for hyper_option in param_list:
            ...
            new_config[ConfigKW.PATH_OUTPUT] = initial_config[ConfigKW.PATH_OUTPUT] + hyper_option.name
            config_list.append(new_config)
    ...
    with ctx.Pool(processes=len(initial_config[ConfigKW.GPU_IDS])) as pool:
        for i in range(n_iterations):
            ...
            if run_test:
                ...
                for config in config_list:
                    # Delete path_pred
                    path_pred = os.path.join(config[ConfigKW.PATH_OUTPUT], 'pred_masks')
                    ...
                    json_path = os.path.join(config[ConfigKW.PATH_OUTPUT], 'config_file.json')
